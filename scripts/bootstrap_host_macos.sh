...
usage() {
  cat <<'USAGE'
Bootstrap RTMS host on macOS.

Usage:
  bootstrap_host_macos.sh --server-url URL [options]
...
Examples:
  ...
  ./scripts/bootstrap_host_macos.sh --server-url http://172.20.10.3:8000 --mode build-only
USAGE
}
