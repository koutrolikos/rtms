...
INSTALL_DIR="$HOME/rtms-host"
OPENOCD_TARGET_CFG="target/stm32g4x.cfg"

usage() {
  cat <<'USAGE'
Bootstrap RTMS host on Linux.

Usage:
  bootstrap_host_linux.sh --server-url URL [options]
...
Examples:
  ...
  ./scripts/bootstrap_host_linux.sh --server-url http://172.20.10.3:8000 --mode build-only
USAGE
}
...
echo "[3/4] Writing host env file"
ENV_FILE="$INSTALL_DIR/.rtms-env.sh"
cat > "$ENV_FILE" <<ENVVARS
export RTMS_INSTALL_DIR="$INSTALL_DIR"
export RTMS_SERVER_URL="$SERVER_URL"
export RTMS_HOST_DATA_DIR="$INSTALL_DIR/host_data"
export RTMS_SERVER_DATA_DIR="$INSTALL_DIR/server_data"
export RTMS_OPENOCD_TARGET_CFG="$OPENOCD_TARGET_CFG"
export RTMS_HOST_BUILD_CAPABLE=$BUILD_CAPABLE
export RTMS_HOST_FLASH_CAPABLE=$FLASH_CAPABLE
export RTMS_HOST_CAPTURE_CAPABLE=$CAPTURE_CAPABLE
export RTMS_OPENOCD_SCAN_PROBES=1
export RTMS_OPENOCD_SCAN_INTERVAL_SECONDS=10
export RTMS_SIMULATE_HARDWARE=0
export RTMS_SIMULATE_CAPTURE=0
export RTMS_OPENOCD_RTT_SEARCH_ADDRESS="0x20000000"
export RTMS_OPENOCD_RTT_SEARCH_SIZE_BYTES=131072
export RTMS_OPENOCD_RTT_ID="SEGGER RTT"
export RTMS_OPENOCD_RTT_HUMAN_CHANNEL=0
export RTMS_OPENOCD_RTT_MACHINE_CHANNEL=1
