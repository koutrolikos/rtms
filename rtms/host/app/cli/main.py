...
from rtms.host.app.core.config import get_settings
from rtms.host.app.core.logging import configure_logging
from rtms.host.app.services.high_altitude_cc_build import main as build_high_altitude_cc_main
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="RF range-test host")
    subparsers = parser.add_subparsers(dest="command", required=True)

    subparsers.add_parser("run", help="Run the polling host")
    ...
    subparsers.add_parser("probe-scan", help="List connected probes visible to this host host")
    return parser
...
def main() -> None:
    ...
    runtime = HostRuntime(get_settings())
    try:
        if args.command == "run":
            runtime.run()
