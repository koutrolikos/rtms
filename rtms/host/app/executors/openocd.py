...
from rtms.host.app.services.probes import ProbeInventorySnapshot, normalize_probe_serial, scan_probe_inventory
from rtms.host.app.storage.local_state import LocalStateStore, PreparedRoleContext
from rtms.shared.enums import RawArtifactType, Role
from rtms.shared.manifest import ArtifactBundleManifest
from rtms.shared.schemas import JobResult, PrepareRolePayload
...
class OpenOcdExecutor:
    def __init__(self, settings: HostSettings, state_store: LocalStateStore) -> None:
    ...
    def prepare(
        ...
        *,
        ...
        if not self.settings.openocd.simulate_hardware:
                ...
                return JobResult(
                    ...
                    diagnostics={
                        **probe_inventory.diagnostics(),
                        "hint": "Connect a single probe or set RTMS_PROBE_SERIAL on this host.",
                    },
            ...
            if probe_inventory.selection_reason in {"no_probes_detected", "probe_scan_failed"} and probe_serial is None:
                return JobResult(
                    ...
                    diagnostics={
                        **probe_inventory.diagnostics(),
                        "hint": "Connect a probe to this host before starting the session.",
                    },
                    time_samples=time_samples,
                )
        ...
        event_log_path = work_dir / "host_events.jsonl"
        ...
        try:
            ...
            raise PrepareFailure(
                "openocd_launch_failed",
                {
                    ...
                    "hint": (
                        "OpenOCD executable was not found. Install OpenOCD on the host host "
                        "or set RTMS_OPENOCD_BIN to the full path of the binary, for example "
