...
class Host(Base):
    ...
    capabilities: Mapped[dict] = mapped_column("capabilities_json", JSON, default=dict)
    diagnostics: Mapped[dict] = mapped_column("diagnostics_json", JSON, default=dict)
...
class RunSession(Base):
    ...
    tx_host_id: Mapped[str | None] = mapped_column("tx_agent_id", String(36), nullable=True)
    rx_host_id: Mapped[str | None] = mapped_column("rx_agent_id", String(36), nullable=True)
...
class Artifact(Base):
    ...
    producing_host_id: Mapped[str | None] = mapped_column("producing_agent_id", String(36), nullable=True)
    role_compatibility: Mapped[list] = mapped_column("role_compatibility_json", JSON, default=list)
    hash_sha256: Mapped[str | None] = mapped_column(String(128), nullable=True)
    metadata_payload: Mapped[dict] = mapped_column("metadata_json", JSON, default=dict)
...
class SessionRoleRun(Base):
    ...
    host_id: Mapped[str] = mapped_column("agent_id", String(36))
...
class Job(Base):
    ...
    host_id: Mapped[str] = mapped_column("agent_id", String(36), index=True)
    role: Mapped[str | None] = mapped_column(String(8), nullable=True, index=True)
    type: Mapped[str] = mapped_column(String(64), index=True)
    status: Mapped[str] = mapped_column(String(32), default="pending", index=True)
    payload: Mapped[dict] = mapped_column("payload_json", JSON, default=dict)
    result: Mapped[dict] = mapped_column("result_json", JSON, default=dict)
    diagnostics: Mapped[dict] = mapped_column("diagnostics_json", JSON, default=dict)
    failure_reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, onupdate=utc_now)
