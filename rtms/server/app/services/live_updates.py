...
def hosts_change_token(db: Session) -> str:
    host_count, latest_host_change = _count_and_latest(db.query(Host), Host.updated_at)
    session_count, latest_session_change = _count_and_latest(db.query(RunSession), RunSession.updated_at)
    latest_change = max(
        (
            value
            for value in (
...
def session_change_token(db: Session, session: RunSession) -> str:
