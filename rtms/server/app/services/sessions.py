...
from rtms.shared.state_machine import transition_job, transition_role_run, transition_session
...
def canonical_raw_artifact_storage_path(
    *,
    ...
    singleton_filenames = {
        ...
        RawArtifactType.HOST_EVENT_LOG: "host_events.jsonl",
...
def delete_terminal_session(
    ...
    *,
    ...
    db.query(RunSession).filter(RunSession.id == session_id).delete(synchronize_session=False)
    db.commit()
...
def create_session(db: Session, settings: ServerSettings, request: SessionCreateRequest) -> RunSession:
    session = RunSession(
        name=request.name,
...
def update_session_metadata(db: Session, session_id: str, request: SessionUpdateRequest) -> RunSession:
    session = get_session_or_404(db, session_id)
...
def get_session_or_404(db: Session, session_id: str) -> RunSession:
    session = db.get(RunSession, session_id)
    if session is None:
        raise HTTPException(status_code=404, detail="session not found")
    return session


def list_sessions(db: Session) -> list[RunSession]:
    return db.query(RunSession).order_by(RunSession.created_at.desc()).all()


def get_active_session(db: Session) -> RunSession | None:
    return (
        db.query(RunSession)
        .filter(
...
def _reconcile_preflight_state(session: RunSession) -> None:
    if session.status in {
        ...
        return
    if session.tx_artifact_id and session.rx_artifact_id and session.tx_host_id and session.rx_host_id:
...
def assign_artifact(db: Session, session_id: str, request: AssignArtifactRequest) -> RunSession:
    session = get_session_or_404(db, session_id)
...
def request_build(
    ...
    *,
    ...
    artifact = Artifact(
        ...
        producing_host_id=request.build_host_id,
        role_compatibility=[request.role.value],
        metadata_payload={
            "repo_id": repo.id,
...
def create_manual_artifact_record(
    ...
    *,
    ...
    artifact = Artifact(
        ...
        role_compatibility=[role_hint.value] if role_hint else [],
    )
    db.add(artifact)
    db.commit()
    db.refresh(artifact)
...
def _ensure_role_runs_exist(db: Session, session: RunSession) -> None:
    for role, host_id, artifact_id in (
        (Role.TX, session.tx_host_id, session.tx_artifact_id),
        (Role.RX, session.rx_host_id, session.rx_artifact_id),
    ):
        role_run = _role_run_for(db, session.id, role, required=False)
        if role_run is None and host_id:
...
def _assert_session_startable(db: Session, session: RunSession) -> None:
...
def _capture_duration_seconds(session: RunSession) -> int | None:
...
def stop_session(db: Session, session_id: str) -> RunSession:
    session = get_session_or_404(db, session_id)
...
def _dispatch_capture_start(db: Session, settings: ServerSettings, session: RunSession) -> None:
...
def register_raw_artifact(
    ...
    *,
    ...
    raw.metadata_payload = metadata or {}
    ...
    log_event(
        db,
        session_id=session_id,
        source_type=EventSourceType.HOST,
        source_ref=raw.id,
...
def apply_artifact_upload(
    ...
    *,
    ...
    artifact.metadata_payload = {**(artifact.metadata_payload or {}), **metadata}
    artifact.status = ArtifactStatus.READY.value
    if producing_host_id:
        artifact.producing_host_id = producing_host_id
    db.commit()
    db.refresh(artifact)
    session = db.get(RunSession, artifact.session_id)
    if session is not None:
        auto_assign = (artifact.metadata_payload or {}).get("auto_assign_role")
        ...
        if session.status == SessionState.BUILDING_ARTIFACTS.value:
            session.status = transition_session(
                ...
                if session.tx_host_id and session.rx_host_id and session.tx_artifact_id and session.rx_artifact_id
...
def handle_job_result(
    ...
    *,
    ...
    db.commit()
    session = db.get(RunSession, job.session_id) if job.session_id else None
    if session is not None:
        log_event(
            db,
            session_id=session.id,
            source_type=EventSourceType.HOST,
            source_ref=job.host_id,
            event_type=EventType.JOB_UPDATE,
            payload={
                "job_id": job.id,
                "job_type": job.type,
    ...
    if job.type == JobType.BUILD_ARTIFACT.value:
        artifact_id = result.artifact_id or job.payload.get("artifact_id")
        if artifact_id:
            artifact = db.get(Artifact, artifact_id)
            if artifact is not None:
                ...
                artifact.metadata_payload = metadata
